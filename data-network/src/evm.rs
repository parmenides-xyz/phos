...
pub struct DataNetworkEvm<E: ExecutionProvider<DataNetwork>> {
    execution: Arc<E>,
    chain_id: u64,
    block_id: BlockId,
    fork_schedule: ForkSchedule,
    phantom: PhantomData<DataNetwork>,
}

impl<E: ExecutionProvider<DataNetwork>> DataNetworkEvm<E> {
    pub fn new(
        execution: Arc<E>,
        chain_id: u64,
        fork_schedule: ForkSchedule,
        ...
        let tx_res = loop {
            ...
            let (result, needs_update) = {
                let mut evm = context
                    .with_db(&mut db)
                    .build_mainnet()
                    .with_precompiles(DataNetworkPrecompiles::default());
                let res = evm.replay();
    ...
    fn get_context(
        ...
        if <TxType as Into<u8>>::into(
            <TransactionRequest as TransactionBuilder<DataNetwork>>::output_tx_type(tx),
        ) == 0u8
        {
    ...
    fn tx_env(tx: &TransactionRequest, spec: SpecId) -> TxEnv {
        ...
        TxEnv {
            ...
            gas_limit: <TransactionRequest as TransactionBuilder<DataNetwork>>::gas_limit(tx)
                .unwrap_or(default_gas_limit),
            gas_price: <TransactionRequest as TransactionBuilder<DataNetwork>>::gas_price(tx)
                .unwrap_or_default(),
            kind: tx.to.unwrap_or_default(),
            value: tx.value.unwrap_or_default(),
            data: <TransactionRequest as TransactionBuilder<DataNetwork>>::input(tx)
                .unwrap_or_default()
                .clone(),
            nonce: <TransactionRequest as TransactionBuilder<DataNetwork>>::nonce(tx)
                .unwrap_or_default(),
            chain_id: <TransactionRequest as TransactionBuilder<DataNetwork>>::chain_id(tx),
            access_list: <TransactionRequest as TransactionBuilder<DataNetwork>>::access_list(tx)
                .cloned()
                .unwrap_or_default(),
            gas_priority_fee:
                <TransactionRequest as TransactionBuilder<DataNetwork>>::max_priority_fee_per_gas(
                    tx,
                ),
