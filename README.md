...
Light client for DATA Network (formerly Story), written in Rust.

## Why a light client?
Homer converts an untrusted RPC endpoint into a safe unmanipulable local RPC for users. It syncs in seconds, requires no storage, and is lightweight enough to run in browser + on mobile devices.
